...
class Config(object):
    ...
    def config_req(self, kw):
        ...
        for item in kw:
            ...
            if item["method"] == "get":
                ...
                print("现在只针对post和get方法进行了测试，其他方法请自行扩展")
            app["url"] = item["url"]
    ...
    def config_pict(selfself, kw, req=None):
        app = {}
        header = {
            "Accept": "*/*",
            "Content-Type": "application/json;charset=utf-8"}
        for item in kw:
            url = "%s://%s" % (item["protocol"], item["url"])
            params = BaseParams().param_fi(
                ast.literal_eval(item["params"]))["params"]
            for i in params:
                _info = ""
    ...
    def __chenk(self, hope, res):
        hope = json.loads(hope)
        fact = json.loads(res)
        for items in fact:
            if isinstance(fact[items], list):
                for item in fact[items]:
                    for k in hope:
                        if item.get(k, "") == hope[k]:
                            return "通过"
            if isinstance(fact[items], dict):
                for k in hope:
