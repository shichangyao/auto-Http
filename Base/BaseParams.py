...
class BaseParams():
    ...
    def param_fi(self,param):
        ...
        self.__pict_wire()
        time.sleep(2)
        pcit_param = self.__read_pcit_param()
        o_param = self.__filter_pict_param(param,pcit_param)
            ...
            for i in item:
                for key in i:
                    if key == "info":
                        ...
                        _app[key] = i[key]
                    _app["info"] = _info
            _result["param"].append(_app)
        ls["info"] = "全部参数正确"
        _result["param"].append(ls)
